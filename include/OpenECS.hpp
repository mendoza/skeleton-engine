...
/// This is how an assertion is done. Can be defined with something else if
/// needed.
#ifndef ECS_ASSERT
...
#define ECS_ASSERT_ENTITY_CORRECT_SIZE(T)                                      \
  static_assert(                                                               \
      sizeof(details::BaseEntity) == sizeof(T), #T                             \
      " should not include new variables, add them as Components instead.");
...
// we specialize for pointers to member function
{
  ...
  template <size_t i> struct arg_t {
    typedef typename std::tuple_element<i, std::tuple<Args...>>::type type;
  };
  template <size_t i> using arg = typename arg_t<i>::type;

  template <size_t i> struct arg_remove_ref_t {
    typedef typename std::remove_reference<arg<i>>::type type;
  };
  template <size_t i> using arg_remove_ref = typename arg_remove_ref_t<i>::type;
...
template <typename T> struct is_type<T, T> : std::true_type {};
...
inline size_t inc_component_counter() {
  ...
  ECS_ASSERT(index < ECS_MAX_NUM_OF_COMPONENTS,
             "maximum number of components exceeded.");
  return index;
}

template <typename C> size_t component_index() {
...
template <typename C> size_t system_index() {
...
template <typename C>
inline auto component_mask() ->
    typename std::enable_if<!std::is_same<C, Entity>::value,
                            ComponentMask>::type {
...
inline auto component_mask() ->
    typename std::enable_if<!std::is_same<C1, Entity>::value,
                            ComponentMask>::type {
...
public:
  inline explicit BasePool(size_t element_size,
                           size_t chunk_size = ECS_DEFAULT_CHUNK_SIZE);
...
BasePool::BasePool(size_t element_size, size_t chunk_size)
    : size_(0), capacity_(0), element_size_(element_size),
...
template <typename T> void Pool<T>::destroy(index_t index) {
  ECS_ASSERT(index < size_, "Pool has not allocated memory for this index.");
  T *ptr = get_ptr(index);
  ptr->~T();
}

template <typename T> inline T *Pool<T>::get_ptr(index_t index) {
  ECS_ASSERT(index < capacity_,
             "Pool has not allocated memory for this index.");
  return reinterpret_cast<T *>(chunks_[index / chunk_size_] +
                               (index % chunk_size_) * element_size_);
}

template <typename T> inline const T *Pool<T>::get_ptr(index_t index) const {
  ECS_ASSERT(index < this->capacity_,
             "Pool has not allocated memory for this index.");
  return reinterpret_cast<T *>(chunks_[index / chunk_size_] +
                               (index % chunk_size_) * element_size_);
}

template <typename T> inline T &Pool<T>::get(index_t index) {
  return *get_ptr(index);
}

template <typename T> inline const T &Pool<T>::get(index_t index) const {
  return *get_ptr(index);
}

template <typename T> inline T &Pool<T>::operator[](size_t index) {
  return get(index);
}

template <typename T> inline const T &Pool<T>::operator[](size_t index) const {
  return get(index);
}
...
// Creating a component that doesn't have ctor, and is not a property -> create
...
// Creating a component that doesn't have ctor, and is a property -> create
// using underlying Property ctor
template <typename C, typename... Args>
auto create_component(void *ptr, Args &&... args) -> typename std::enable_if<
    !std::is_constructible<C, Args...>::value &&
        std::is_base_of<details::BaseProperty, C>::value,
...
public:
  ...
  template <typename... Args> C &create(index_t index, Args &&... args);
...
template <typename...> class EntityAlias;
class Entity;
...
/// function.
template <size_t N, typename...> struct with_t;
...
public:
  ...
  /// Create a specified number of new entities, and do something with each
  /// entity
  template <typename T>
  ...
  inline auto create_with(Args &&... args) ->
      typename std::conditional<(sizeof...(Components) > 0),
                                EntityAlias<Components...>,
                                EntityAlias<Args...>>::type;
  ...
  // Iterate through all entities with all components, specified as lambda
  // parameters example: entities.with([] (Position& pos) {  });
  template <typename T> inline void with(T lambda);

  // Access a View of all entities that has every component as Specified
  // EntityAlias
  template <typename T> inline View<T> fetch_every();

  // Access a View of all entities that has every component as Specified
  // EntityAlias specified as lambda parameters example: entities.fetch_every([]
  // (EntityAlias<Position>& entity) {  });
  template <typename T> inline void fetch_every(T lambda);
...
private:
  ...
  /// with similar components
  inline std::vector<Entity> create_with_mask(details::ComponentMask mask,
                                              const size_t num_of_entities);
  ...
  inline void create_new_block(IndexAccessor &index_accessor,
                               unsigned long mask_as_ulong,
                               index_t next_free_index);

  /// Creates a ComponentManager. Mainly used by get_component_manager the first
  /// time its called
  template <typename C, typename... Args>
  inline details::ComponentManager<C> &
  create_component_manager(Args &&... args);

  /// Get the ComponentManager. Assumes that the component manager already
  /// exists.
  template <typename C>
  ...
  inline details::BaseManager const &
  get_component_manager(size_t component_index) const;

  /// Get component for a specific entity or index.
  template <typename C> inline C &get_component(Entity &entity);
  template <typename C>
  ...
  template <typename C> inline C &get_component_fast(index_t index);
  template <typename C> inline C const &get_component_fast(index_t index) const;
  template <typename C> inline C &get_component_fast(Entity &entity);
  template <typename C>
  ...
  /// Use to create a component tmp that is assignable. Uses uniform
  /// initialization.
  template <typename C, typename... Args>
  inline static auto create_tmp_component(Args &&... args) ->
  ...
  /// Use to create a component tmp that is assignable. Call the right
  /// constructor Called when component is a property, and no constructor
  /// inaccessible.
  template <typename C, typename... Args>
  inline static auto create_tmp_component(Args &&... args) ->
      typename std::enable_if<
    ...
    static_assert(sizeof...(Args) == 1,
                  ECS_ASSERT_MSG_ONLY_ONE_ARGS_PROPERTY_CONSTRUCTOR);
  ...
  template <typename C> inline void remove_component(Entity &entity);

  /// Removes a specific component from an entity. Assumes that
  /// a ComponentManager exists for component type
  template <typename C> inline void remove_component_fast(Entity &entity);
  ...
  inline bool has_component(Entity &entity,
                            details::ComponentMask component_mask);
  inline bool has_component(Entity const &entity,
                            details::ComponentMask const &component_mask) const;
  template <typename... Components> inline bool has_component(Entity &entity);
  template <typename... Components>
  ...
  /// Get an entity given its id or index. When using Id, it must always be
  /// valid
  ...
  /// How many blocks of entities there exists. Used when tracking where to put
  /// entities in momory
  ...
  template <size_t N, typename...> friend struct details::with_t;
  template <typename T> friend class details::ComponentManager;
  template <typename... Cs> friend class EntityAlias;
...
public:
  ...
  /// Add the requested component, error if component of the same type exist
  /// already
  ...
  /// Clears the component mask without destroying components (faster than
  /// remove_everything)
...
private:
  ...
  template <typename C> using is_component = details::is_type<C, Components...>;
...
public:
  ...
  /// Removes a component. Error of it doesn't exist. Cannot remove dependent
  /// components
  template <typename C>
  inline auto remove() ->
...
private:
  ...
  template <typename C0, typename Arg> inline void init_components(Arg arg) {
    add<C0>(arg);
  }

  template <typename C0, typename C1, typename... Cs, typename Arg0,
  ...
  template <typename C> inline void init_components() { add<C>(); }
  ...
  template <typename... Args> inline void init(Args... args) {
...
template <typename... Cs> EntityAlias<Cs...>::EntityAlias() {}

template <typename... Cs>
EntityAlias<Cs...>::EntityAlias(const Entity &entity)
    : details::BaseEntity(entity) {}

template <typename... Cs> EntityAlias<Cs...>::operator Entity &() {
  return entity();
}

template <typename... Cs> EntityAlias<Cs...>::operator Entity const &() const {
  return entity();
}

template <typename... Cs> Id &EntityAlias<Cs...>::id() { return entity().id(); }

template <typename... Cs> Id const &EntityAlias<Cs...>::id() const {
  return entity().id();
}
...
inline auto EntityAlias<Cs...>::set(Args &&... args) ->
    typename std::enable_if<is_component<C>::value, C &>::type {
  return entities().template set_component_fast<C>(entity(),
...
inline auto EntityAlias<Cs...>::set(Args &&... args) ->
    typename std::enable_if<!is_component<C>::value, C &>::type {
  return entities().template set_component<C>(entity(),
                                              std::forward<Args>(args)...);
}
...
template <typename... Cs> template <typename C> C &EntityAlias<Cs...>::as() {
...
template <typename... Cs> void EntityAlias<Cs...>::remove_everything() {
  entity().remove_everything();
}

template <typename... Cs> void EntityAlias<Cs...>::clear_mask() {
  entity().clear_mask();
}

template <typename... Cs> void EntityAlias<Cs...>::destroy() {
  entity().destroy();
}
...
template <typename... Cs> template <typename T> bool EntityAlias<Cs...>::is() {
...
template <typename... Cs> bool EntityAlias<Cs...>::is_valid() {
  return entity().is_valid();
}

template <typename... Cs> bool EntityAlias<Cs...>::is_valid() const {
  return entity().is_valid();
}
...
template <typename C> C const &Entity::get() const {
  return manager_->get_component<C>(*this);
}

template <typename C, typename... Args> C &Entity::set(Args &&... args) {
  return manager_->set_component<C>(*this, std::forward<Args>(args)...);
}

template <typename C, typename... Args> C &Entity::add(Args &&... args) {
  return manager_->create_component<C>(*this, std::forward<Args>(args)...);
}

template <typename T> inline T &Entity::as() {
  ECS_ASSERT_IS_ENTITY(T);
  ECS_ASSERT_ENTITY_CORRECT_SIZE(T);
  ECS_ASSERT(has(T::static_mask()),
             "Entity doesn't have required components for this EntityAlias");
  return reinterpret_cast<T &>(*this);
}

template <typename T> inline T const &Entity::as() const {
  ECS_ASSERT_IS_ENTITY(T);
  ECS_ASSERT_ENTITY_CORRECT_SIZE(T);
  ECS_ASSERT(has(T::static_mask()),
             "Entity doesn't have required components for this EntityAlias");
...
void Entity::remove_everything() { manager_->remove_all_components(*this); }

void Entity::clear_mask() { manager_->clear_mask(*this); }

void Entity::destroy() { manager_->destroy(*this); }

template <typename... Components> bool Entity::has() {
  return manager_->has_component<Components...>(*this);
}

template <typename... Components> bool Entity::has() const {
  return manager_->has_component<Components...>(*this);
}

template <typename T> bool Entity::is() {
  ECS_ASSERT_IS_ENTITY(T);
  ECS_ASSERT_ENTITY_CORRECT_SIZE(T);
  return has(T::static_mask());
}

template <typename T> bool Entity::is() const {
  ECS_ASSERT_IS_ENTITY(T);
  ECS_ASSERT_ENTITY_CORRECT_SIZE(T);
  return has(T::static_mask());
}

bool Entity::is_valid() { return manager_->is_valid(*this); }

bool Entity::is_valid() const { return manager_->is_valid(*this); }
...
details::ComponentMask &Entity::mask() { return manager_->mask(*this); }
...
public:
  ...
  /// Allocates memory for this Entity. Once allocated, the Unallocated Entity
  /// function like a normal Entity
...
UnallocatedEntity::~UnallocatedEntity() { allocate(); }
...
template <typename C> C &UnallocatedEntity::get() {
...
template <typename C> C const &UnallocatedEntity::get() const {
  ECS_ASSERT(is_valid(), "Unallocated Entity invalid");
  ECS_ASSERT(has<C>(), "UnallocatedEntity does not have Component Attached");
  if (is_allocated()) {
    return entity_.get<C>();
...
C &UnallocatedEntity::set(Args &&... args) {
  ...
  if (has<C>()) {
    return get<C>() =
               manager_->create_tmp_component<C>(std::forward<Args>(args)...);
  } else {
...
C &UnallocatedEntity::add(Args &&... args) {
  ...
  ECS_ASSERT(!has<C>(), "Unallocated Entity cannot assign already assigned "
                        "component with add. Use set instead");
  ECS_ASSERT(is_valid(), "Unallocated Entity invalid");
  int index = 0;
  ...
  component_headers_.push_back(
      ComponentHeader{static_cast<unsigned int>(component_index), sizeof(C)});
  component_data.resize(index + sizeof(C));
  return details::create_component<C>(&component_data[index],
                                      std::forward<Args>(args)...);
}

template <typename C> C &UnallocatedEntity::as() {
  allocate();
  return entity_.as<C>();
}
...
template <typename C> void UnallocatedEntity::remove() {
  if (is_allocated()) {
    entity_.remove<C>();
  } else {
    int index = 0;
...
template <typename... Components> bool UnallocatedEntity::has() const {
  if (is_allocated()) {
...
template <typename T> bool UnallocatedEntity::is() const {
  if (is_allocated()) {
    return entity_.is<T>();
  } else {
    ECS_ASSERT_IS_ENTITY(T);
...
inline bool operator==(const UnallocatedEntity &lhs,
                       const UnallocatedEntity &rhs) {
  return &lhs == &rhs;
}
...
bool UnallocatedEntity::is_allocated() const { return manager_ == nullptr; }
...
void UnallocatedEntity::allocate() {
  if (!is_allocated()) {
    ...
    if (component_headers_.size() > 0) {
      ...
      // TODO: set mask
      for (auto componentHeader : component_headers_) {
        details::BaseManager &componentManager =
            manager_->get_component_manager(componentHeader.index);
        componentManager.ensure_min_size(index);
        // Copy data from tmp location to acctuial location in component manager
        std::memcpy(componentManager.get_void_ptr(index),
                    &component_data[offset], componentHeader.size);
...
struct with_t<N, Lambda, Args...>
    : with_t<N - 1, Lambda,
             typename details::function_traits<Lambda>::template arg_remove_ref<
                 N - 1>,
             Args...> {};

template <typename Lambda, typename... Args> struct with_t<0, Lambda, Args...> {
  static inline void for_each(EntityManager &manager, Lambda lambda) {
    typedef details::function_traits<Lambda> function;
    static_assert(function::arg_count > 0,
                  "Lambda or function must have at least 1 argument.");
...
UnallocatedEntity EntityManager::create() { return UnallocatedEntity(*this); }
...
inline void EntityManager::create(const size_t num_of_entities, T lambda) {
  ...
  using EntityAlias_ =
      typename details::function_traits<T>::template arg_remove_ref<0>;
  ECS_ASSERT_IS_ENTITY(EntityAlias_);
  for (Entity &entity :
       create_with_mask(EntityAlias_::static_mask(), num_of_entities)) {
    // We cannot use as<EntityAlias> since we dont have attached any components
    lambda(*reinterpret_cast<EntityAlias_ *>(&entity));
    ECS_ASSERT(entity.has(EntityAlias_::static_mask()),
               "Entity are missing certain components.");
  }
}
...
auto EntityManager::create_with(Args &&... args) ->
    typename std::conditional<(sizeof...(Components) > 0),
                              EntityAlias<Components...>,
                              EntityAlias<Args...>>::type {
  using Type = typename std::conditional<(sizeof...(Components) > 0),
                                         EntityAlias<Components...>,
                                         EntityAlias<Args...>>::type;
...
std::vector<Entity>
EntityManager::create_with_mask(details::ComponentMask mask,
                                const size_t num_of_entities) {
  ...
  IndexAccessor &index_accessor =
      component_mask_to_index_accessor_[mask_as_ulong];
  ...
  index_t current =
      ECS_CACHE_LINE_SIZE; // <- if empty, create new block instantly
  size_t slots_required;
  // Are there any unfilled blocks?
  if (!index_accessor.block_index.empty()) {
    block_index =
        index_accessor.block_index[index_accessor.block_index.size() - 1];
    current = next_free_indexes_[block_index];
    slots_required =
        block_count_ * ECS_CACHE_LINE_SIZE + current + entities_left;
  } else {
  ...
  while (entities_left) {
    for (; current < ECS_CACHE_LINE_SIZE && entities_left; ++current) {
      new_entities.push_back(
          get_entity(current + ECS_CACHE_LINE_SIZE * block_index));
      entities_left--;
    }
...
template <typename T> void EntityManager::with(T lambda) {
  ECS_ASSERT_IS_CALLABLE(T);
  details::with_<T>::for_each(*this, lambda);
}

template <typename T> View<T> EntityManager::fetch_every() {
  ECS_ASSERT_IS_ENTITY(T);
  return View<T>(this, T::static_mask());
}

template <typename T> void EntityManager::fetch_every(T lambda) {
  ECS_ASSERT_IS_CALLABLE(T);
  typedef details::function_traits<T> function;
  static_assert(function::arg_count == 1,
                "Lambda or function must only have one argument");
...
Entity EntityManager::operator[](index_t index) { return get_entity(index); }
...
size_t EntityManager::count() { return count_; }
...
void EntityManager::create_new_block(
    EntityManager::IndexAccessor &index_accessor, unsigned long mask_as_ulong,
    index_t next_free_index) {
...
template <typename C, typename... Args>
details::ComponentManager<C> &
EntityManager::create_component_manager(Args &&... args) {
  details::ComponentManager<C> *ptr = new details::ComponentManager<C>(
      std::forward<EntityManager &>(*this), std::forward<Args>(args)...);
...
details::ComponentManager<C> &EntityManager::get_component_manager_fast() {
  return *reinterpret_cast<details::ComponentManager<C> *>(
      component_managers_[details::component_index<C>()]);
}

template <typename C>
details::ComponentManager<C> const &
EntityManager::get_component_manager_fast() const {
  return *reinterpret_cast<details::ComponentManager<C> *>(
      component_managers_[details::component_index<C>()]);
}
...
details::ComponentManager<C> &EntityManager::get_component_manager() {
  ...
  return *reinterpret_cast<details::ComponentManager<C> *>(
      component_managers_[index]);
}

template <typename C>
details::ComponentManager<C> const &
EntityManager::get_component_manager() const {
  auto index = details::component_index<C>();
  ECS_ASSERT(component_managers_.size() > index,
             component_managers_[index] == nullptr &&
...
details::BaseManager &
EntityManager::get_component_manager(size_t component_index) {
  ECS_ASSERT(component_managers_.size() > component_index,
             "ComponentManager not created with that component index.");
  return *component_managers_[component_index];
}
details::BaseManager const &
EntityManager::get_component_manager(size_t component_index) const {
  ECS_ASSERT(component_managers_.size() > component_index,
             "ComponentManager not created with that component index.");
  return *component_managers_[component_index];
}

template <typename C> C &EntityManager::get_component(Entity &entity) {
  ECS_ASSERT(has_component<C>(entity),
...
template <typename C> C &EntityManager::get_component_fast(index_t index) {
...
template <typename C> C &EntityManager::get_component_fast(Entity &entity) {
...
C &EntityManager::create_component(Entity &entity, Args &&... args) {
  ...
  ECS_ASSERT(!has_component<C>(entity),
             "Entity already has this component attached");
  C &component = get_component_manager<C>().create(entity.id_.index_,
                                                   std::forward<Args>(args)...);
  entity.mask().set(details::component_index<C>());
  return component;
}

template <typename C> void EntityManager::remove_component(Entity &entity) {
  ECS_ASSERT_VALID_ENTITY(entity);
  ECS_ASSERT(has_component<C>(entity),
...
C &EntityManager::set_component(Entity &entity, Args &&... args) {
  ...
  if (entity.has<C>()) {
    return get_component_fast<C>(entity) =
               create_tmp_component<C>(std::forward<Args>(args)...);
  } else
...
bool EntityManager::has_component(
    Entity const &entity, details::ComponentMask const &component_mask) const {
  ECS_ASSERT_VALID_ENTITY(entity);
  return (mask(entity) & component_mask) == component_mask;
}
...
Entity EntityManager::get_entity(Id id) { return Entity(this, id); }

Entity EntityManager::get_entity(index_t index) {
  return get_entity(Id(index, entity_versions_[index]));
}

size_t EntityManager::capacity() const { return entity_versions_.capacity(); }
...
template <typename C> void ComponentManager<C>::remove(index_t index) {
  pool_.destroy(index);
  manager_.mask(index).reset(component_index<C>());
}

template <typename C> C &ComponentManager<C>::operator[](index_t index) {
  return get(index);
}

template <typename C> C &ComponentManager<C>::get(index_t index) {
  return *get_ptr(index);
}

template <typename C> C const &ComponentManager<C>::get(index_t index) const {
  return *get_ptr(index);
}

template <typename C> C *ComponentManager<C>::get_ptr(index_t index) {
...
template <typename C> void *ComponentManager<C>::get_void_ptr(index_t index) {
  return pool_.get_ptr(index);
}
...
template <typename C> void ComponentManager<C>::ensure_min_size(index_t size) {
  pool_.ensure_min_size(size);
}

template <typename C> ComponentMask ComponentManager<C>::mask() {
...
///---------------------------------------------------------------------
template <typename T> struct Property : details::BaseProperty {
  Property() {}
...
template <typename T> inline T &operator++(Property<T> &lhs) {
  ++lhs.value;
  return lhs.value;
}

template <typename T> inline T operator++(Property<T> &lhs, int) {
  T copy = lhs;
  ++lhs;
  return copy;
}
...
template <typename T, typename E> inline T operator~(Property<T> &lhs) {
  return ~lhs.value;
}
...
template <typename T> istream &operator>>(istream &is, ecs::Property<T> &obj) {
  return is >> obj.value;
}
...
public:
  Iterator(EntityManager *manager, details::ComponentMask mask,
           bool begin = true);
...
template <typename T>
Iterator<T>::Iterator(EntityManager *manager, details::ComponentMask mask,
                      bool begin)
...
template <typename T> index_t Iterator<T>::index() const { return cursor_; }

template <typename T> inline void Iterator<T>::find_next() {
  while (cursor_ < size_ &&
         (manager_->component_masks_[cursor_] & mask_) != mask_) {
    ++cursor_;
  }
}

template <typename T> T Iterator<T>::entity() {
  return manager_->get_entity(index())
      .template as<typename Iterator<T>::T_no_ref>();
}

template <typename T> const T Iterator<T>::entity() const {
  return manager_->get_entity(index())
      .template as<typename Iterator<T>::T_no_ref>();
}
...
public:
  ...
  index_t count();
  template <typename... Components> View<T> &&with();
...
template <typename T> typename View<T>::iterator View<T>::begin() {
  return iterator(manager_, mask_, true);
}

template <typename T> typename View<T>::iterator View<T>::end() {
  return iterator(manager_, mask_, false);
}

template <typename T> typename View<T>::const_iterator View<T>::begin() const {
  return const_iterator(manager_, mask_, true);
}

template <typename T> typename View<T>::const_iterator View<T>::end() const {
  return const_iterator(manager_, mask_, false);
}

template <typename T> inline index_t View<T>::count() {
  index_t count = 0;
...
EntityManager &System::entities() { return *manager_->entities_; }
...
template <typename S, typename... Args> S &SystemManager::add(Args &&... args) {
...
template <typename S> void SystemManager::remove() {
  ECS_ASSERT_IS_SYSTEM(S);
...
template <typename S> inline bool SystemManager::exists() {
  ECS_ASSERT_IS_SYSTEM(S);
  return systems_.size() > details::system_index<S>() &&
         systems_[details::system_index<S>()] != nullptr;
}
