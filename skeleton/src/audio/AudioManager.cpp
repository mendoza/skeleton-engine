...
void AudioManager::init() {
  if (SDL_Init(SDL_INIT_AUDIO) < 0) {
    skeleton::core::Logger::error("SDL audio init failed: " +
                                  std::string(SDL_GetError()));
    return;
  }
  if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0)
    skeleton::core::Logger::error("SDL_mixer init failed: " +
...
void AudioManager::bind_sound(const std::string &name,
...
void AudioManager::bind_music(const std::string &name,
                              const std::string &path) {
  if (music_.count(name))
    return;
...
void bind_audio(sol::state &lua) {
  lua.new_usertype<AudioManager>(
      "__AudioManager", "bind_sound", &AudioManager::bind_sound, "bind_music",
      &AudioManager::bind_music, "play",
      sol::overload(
          [](AudioManager &am, const std::string &name) { am.play(name); }),
      "play_music",
      sol::overload([](AudioManager &am, const std::string &name) {
        am.play_music(name);
      }),
      "stop_music", &AudioManager::stop_music, "pause_music",
      &AudioManager::pause_music, "resume_music", &AudioManager::resume_music,
      "set_sound_volume", &AudioManager::set_sound_volume, "set_music_volume",
