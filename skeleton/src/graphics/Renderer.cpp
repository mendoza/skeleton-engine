...
void Renderer::create_window(std::string title, int width, int height, bool debug_mode) {
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        skeleton::core::Logger::error("SDL could not initialize: " + std::string(SDL_GetError()));
        return;
    }
...
void Renderer::draw_rect(skeleton::Rect rect, skeleton::Color color) {
    ...
    SDL_Rect sdl_rect = {(int)pos.x, (int)pos.y, (int)(rect.w * zoom), (int)(rect.h * zoom)};
...
void Renderer::draw_texture(size_t key, skeleton::Rect *src, skeleton::Rect *dst) {
    ...
    if (src) { sdl_src = {(int)src->x, (int)src->y, (int)src->w, (int)src->h}; p_src = &sdl_src; }
    if (dst) {
        skeleton::Vec2 pos = to_screen({dst->x, dst->y});
        float zoom = active_camera ? active_camera->zoom : 1.0f;
        sdl_dst = {(int)pos.x, (int)pos.y, (int)(dst->w * zoom), (int)(dst->h * zoom)};
        p_dst = &sdl_dst;
    }
...
void Renderer::draw_line(skeleton::Vec2 a, skeleton::Vec2 b, skeleton::Color color) {
...
void Renderer::set_debug_mode(bool dm) {
    debug_mode = dm;
}
