...
static void
parse_clips(sol::table &def,
            std::unordered_map<std::string, AnimationPlayer::Clip> &out) {
  size_t texture_key =
      skeleton::core::TextureManager::get_instance().load_texture(
          def["texture"]);
  ...
  for (auto &[k, v] : anims) {
    ...
    if (flip == "horizontal")
      clip.flip = skeleton::Flip::Horizontal;
    else if (flip == "vertical")
      clip.flip = skeleton::Flip::Vertical;
...
void AnimationPlayer::play(const std::string &name) {
  if (name == current_)
    return;
  if (clips_.find(name) == clips_.end())
    return;
  current_ = name;
  frame_ = 0;
  elapsed_ = 0.0f;
